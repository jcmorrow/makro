...
import { evaluateFormula, getCellLabel } from '../lib/formula-engine';
...
export function Spreadsheet() {
  ...
  const updateCell = useCallback((cellId: string, formula: string) => {
    setCells(prevCells => {
  ...
  const isCellInSelection = useCallback((cellId: string) => {
  ...
  }, [selectedCell, editingCell, mode, cells, handleFormulaBarSubmit, updateCell, selectionStart, selectionEnd]);
  ...
  return (
    ...
    >
      ...
      <div className="bg-slate-900 border-b border-slate-700 p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-500 text-sm">Array-Programming Spreadsheet</span>
          </div>
          <Button
            variant="ghost"
            size="sm"
        ...
        <div className="flex items-center gap-2">
          <div className="text-slate-400 text-sm font-mono w-32 flex items-center gap-2">
            ...
            <span className={`text-xs px-1.5 py-0.5 rounded ${mode === 'normal' ? 'bg-emerald-600 text-white' : mode === 'visual' ? 'bg-purple-600 text-white' : 'bg-blue-600 text-white'}`}>
      ...
      <div className="flex-1 overflow-auto bg-slate-900">
        <div className="inline-block min-w-full">
          <table className="border-collapse">
            <thead>
              <tr>
                <th className="sticky top-0 left-0 z-20 bg-slate-800 border border-slate-700 w-12 h-8 text-slate-400 text-xs">
      ...
      <div className="bg-slate-900 border-t border-slate-700 px-4 py-2 text-xs text-slate-500">
        <span className="text-emerald-400">Vim Mode:</span> h/j/k/l to navigate • Shift+hjkl for VISUAL mode • i to edit • ESC to exit mode • d/x to delete • y to yank • p to paste • g/G for first/last row • 0/$ for first/last col
      </div>
    </div>
  );
}
...
function Cell({ cellId, cell, isSelected, isEditing, isInSelection, onClick, onDoubleClick, onEdit, onMouseDown, onMouseEnter }: CellProps) {
